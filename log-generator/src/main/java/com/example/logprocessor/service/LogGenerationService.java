...
public class LogGenerationService {
    ...
    private final List<String> sources = Arrays.asList("auth-service", "api-gateway", "user-service",
            "payment-service");

    private final Counter micrometerGeneratedCounter;
    private final Counter micrometerRateLimitedCounter;
    ...
    public LogGenerationService(KafkaProducerService kafkaProducerService,
        ...
        // Register metrics
        this.micrometerGeneratedCounter = Counter.builder("log_events_generated")
                .description("Total number of log events generated")
                .register(meterRegistry);

        this.micrometerRateLimitedCounter = Counter.builder("log_events_rate_limited")
        ...
        while (isRunning) {
            try {
                ...
                if (!rateLimitingService.isAllowed(rateLimitKey, 1, eventsPerThread)) {
                    ...
                    micrometerRateLimitedCounter.increment();
                ...
                micrometerGeneratedCounter.increment(batch.size());
    ...
    private long previousGeneratedCount = 0;

    @Scheduled(fixedRate = 1000)
    public void updateMetrics() {
        long currentCount = generatedCounter.get();
        int currentRate = (int) (currentCount - previousGeneratedCount);
        currentRatePerSecond.set(currentRate);
        previousGeneratedCount = currentCount;

        if (isRunning) {
            logger.info("Current rate: {} events/sec, Total: {}", currentRate, currentCount);
        }
    }
